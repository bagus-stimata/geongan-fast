...
from app.repository.geongan.fdivision_repo import (
    create_fdivision,
    get_all_fdivision,
)
...
router = APIRouter(prefix="/api", tags=["fdivision"])


@router.post("/fdivisions", response_model=FDivisionResponse)
...
@router.get("/fdivisions", response_model=list[FDivisionResponse])
